...
//test
@Test(priority = 26) //validate User Login
public void validateUserLogin () { //test case and description
test = extent.createTest("User should contain login", "validate if user contain login"); //checking if current url contains val_login or not
...
@Test(priority = 27)
public void validateTitle() throws InterruptedException { //test case and description
test = extent.createTest("Title should be OrangeHRM", "validate that Title should be OrangeHRM");
String actualTitle = driver.getTitle();
String expectedTitle = InputValues.expTitle; //verify that the expected result and the actual result matched or not
